...
int main(int argc, char* argv[]) {
  ...
  for (nx=1; nx<argc; nx++) {         /* look for command-line args.. */
    if (strcmp(argv[nx],"-b")==0 && ++nx<argc) {     /* alt IL file.. */
      ...
      if (tmpFile != NULL) if (debug_fseek(tmpFile,0,SEEK_END)==0) {
        len = debug_ftell(tmpFile);                      /* get file size.. */
        if (debug_fseek(tmpFile,0,SEEK_SET)==0) if (len>9) {
          len = len/8+len;            /* allow for line end expansion */
          IL = (char*)malloc(len+1);
          if (IL != NULL) len = debug_fread(IL,1,len,tmpFile);
          IL[len] = '\0';
          IoFileClose(tmpFile);}}
      else debug_printf("Could not open file %s", argv[nx]);}
